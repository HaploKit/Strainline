...
if __name__ == '__main__':
    read_fa, contig_fa, outdir, threads, num_contigs= sys.argv[1:]
    min_cov=5
    ...
    with open(contig_fa, 'r') as fr:
        for line in fr:
            if line.startswith('>'):
                ...
                contig2seq[contig] = line.strip()

    #map all corrected reads to all contigs
    os.system("mkdir -p {}".format(outdir))
    bam = outdir+'/reads2contigs.bam'
    os.system("minimap2 -a" + " --secondary=no -t  " +threads  +" "+ contig_fa + " " + read_fa + \
                  "  2>/dev/null |samtools view -hS -F 2048 -|samtools sort -@ 8 - >" + bam)
    os.system("samtools index {}".format(bam))


    print('## reads alingment finished.')
    params=[]
    for i in range(int(num_contigs)):
        params.append((str(i), read_fa,bam, outdir, min_cov,1,False))
        # contig_fa = 'contig.{}.fa'.format(i)
        # cov_based_misasm_removal(str(i), read_fa, outdir, min_cov, threads=1)
        # clip_based_misasm_removal(read_fa,contig_fa , threads, outdir, min_clip_count=20)
    # clip_based_misasm_removal(read_fa, hap_fa, threads, outdir, min_clip_count=20)
    pool = Pool(int(threads))
    res = pool.map(cov_based_misasm_removal, params, chunksize=1)
