...
def cal_overlap(fasta, outdir, platform, threads, filter=True, min_ovlp_len=500, min_identity=0.6, o=200, r=0.8):
    '''
    ...
    if platform == 'pb':  # TODO: -c is necessary or not?
        minimap = "minimap2 -cx ava-pb -Hk19 -Xw5 -m100 -g10000 --max-chain-skip 25  " + \
                  "-t %s %s %s |cut -f 1-12 |fpa drop -i -m >%s" % (threads, fasta, fasta, paf)

    elif platform == 'ont':
        minimap = "minimap2 -cx ava-ont -k15 -Xw5 -m100 -g10000 -r2000 --max-chain-skip 25  " + \
                  "-t %s %s %s |cut -f 1-12 |fpa drop -i -m >%s" % (threads, fasta, fasta, paf)
    else:
...
if __name__ == '__main__':
    ...
    paf = cal_overlap(top_fa, outdir, platform, threads=40, filter=True, min_ovlp_len=400,
                      min_identity=0.6, o=400, r=0.8)
